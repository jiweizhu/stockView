...
public interface CNDailyDao extends JpaRepository<CNDailyVO, CNDailyKey>, JpaSpecificationExecutor<CNDailyVO>, Serializable {
    Page<CNDailyVO> findByIndexCode(String firstname, Pageable pageable);

    @Query(value = "SELECT index_code FROM cn_daily_price ", nativeQuery = true)
    List<String> findIds();

    @Query(value = "SELECT * FROM cn_daily_price where index_code = ?1 ", nativeQuery = true)
    List<CNDailyVO> findAllById(String index_code);
